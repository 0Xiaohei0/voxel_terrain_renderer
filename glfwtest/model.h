...
unsigned int TextureFromFile(const char* path, const string& directory, const aiScene* scene, bool gamma)
{
    ...
    if (filename.find('*') == 0) {
        ...
        int index = stoi(filename.substr(1)); // Convert "*0" to 0
        if (index < scene->mNumTextures) {
            aiTexture* tex = scene->mTextures[index];
            if (tex->mHeight == 0) {
                // Compressed texture 
                int channels;
                data = stbi_load_from_memory((const unsigned char*)tex->pcData, tex->mWidth, &width, &height, &channels, 0);
                nrComponents = 4;
            }
            else {
                ...
                width = tex->mWidth;
                height = tex->mHeight;
                nrComponents = 4; // Typically, embedded textures are RGBA
            }
        }
    }
    else {
    ...
    if (data) {
        ...
        if (filename.find('*') != 0) {
            // Free data only if it was loaded from a file
        ...
        if (filename.find('*') != 0) {
            stbi_image_free(data); // Prevent memory leaks
        }
    }
